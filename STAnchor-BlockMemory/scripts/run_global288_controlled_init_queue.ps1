...
$QueueName = if ($Smoke) { "downstream_global288_controlled_init_smoke" } else { "downstream_global288_controlled_init" }
...
$Experiments = @(
    @{
        Order = "01"
        Label = "latent48_base_only"
        Config = Join-Path $ProjectRoot "configs\metrla_e5_final_latent48_global288_v1.yaml"
        Checkpoint = Join-Path $ProjectRoot "artifacts\metrla_e5_final_latent48_global288_seed42\pretrain_best_relation.pt"
        Bank = Join-Path $ProjectRoot "artifacts\metrla_bank_e5_final_latent48_global288_local_seed42"
        Mode = "base_only"
    },
    @{
        Order = "02"
        Label = "latent48_pretrained_offset_decay"
        Config = Join-Path $ProjectRoot "configs\metrla_e5_final_latent48_global288_v1.yaml"
        Checkpoint = Join-Path $ProjectRoot "artifacts\metrla_e5_final_latent48_global288_seed42\pretrain_best_relation.pt"
        Bank = Join-Path $ProjectRoot "artifacts\metrla_bank_e5_final_latent48_global288_local_seed42"
...
foreach ($Experiment in $Experiments) {
    ...
    if (-not (Test-Path -LiteralPath $Experiment.Bank -PathType Container)) {
        throw "Missing downstream Bank: $($Experiment.Bank)"
    }
