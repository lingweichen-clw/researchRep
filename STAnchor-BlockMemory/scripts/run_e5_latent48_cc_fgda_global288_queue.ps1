param(
    ...
    [ValidateSet("pretrain", "posttrain", "all")]
...
$Experiments = @(
    @{
        ...
        BankName = "metrla_bank_e5_final_latent48_global288_seed42"
    },
    @{
        Label = "cc_fgda"
        ...
        BankName = "metrla_bank_e5_final_latent48_cc_fgda_global288_seed42"
    }
)
...
if ($Stage -in @("posttrain", "all")) {
    foreach ($Experiment in $Experiments) {
        $PretrainedRoot = Join-Path $ProjectRoot ("artifacts\" + $Experiment.RunName)
        $PretrainedCheckpoint = Join-Path $PretrainedRoot "pretrain_best_relation.pt"
        $RandomRoot = Join-Path $ProjectRoot ("artifacts\convergence\random_controls\" + $Experiment.Label)
        ...
        $RandomBank = Join-Path $ProjectRoot ("artifacts\metrla_bank_e5_final_" + $Experiment.Label + "_global288_random_seed42")
        ...
        foreach ($Path in @($RandomRoot, $PretrainedBank, $RandomBank, $DiagnosticRoot, $VisualizationRoot)) {
            if (Test-Path -LiteralPath $Path) { throw "Refusing to reuse post-training output: $Path" }
        }
        New-Item -ItemType Directory -Force -Path $RandomRoot | Out-Null

        Invoke-PythonStep -Name ("02_random_checkpoint_" + $Experiment.Label) -Arguments @(
            (Join-Path $ProjectRoot "scripts\init_random_checkpoint.py"), "--config", $Experiment.Config,
            "--output", $RandomCheckpoint, "--seed", "42"
        )
        Invoke-PythonStep -Name ("03_build_pretrained_bank_" + $Experiment.Label) -Arguments @(
        ...
        Invoke-PythonStep -Name ("04_build_random_bank_" + $Experiment.Label) -Arguments @(
            (Join-Path $ProjectRoot "scripts\build_bank.py"), "--config", $Experiment.Config,
        ...
        Invoke-PythonStep -Name ("05_diagnose_pretrained_" + $Experiment.Label) -Arguments $DiagnosticArguments
        ...
        Invoke-PythonStep -Name ("06_diagnose_random_" + $Experiment.Label) -Arguments $RandomDiagnosticArguments
        ...
        Invoke-PythonStep -Name ("07_visualize_" + $Experiment.Label) -Arguments $VisualizationArguments

        $Branches = @(
            @{ Order = "08"; Label = "base_only"; Mode = "base_only"; Checkpoint = $PretrainedCheckpoint; Bank = $PretrainedBank },
            @{ Order = "09"; Label = "pretrained_offset_decay"; Mode = "learned_topk_offset_decay_horizon"; Checkpoint = $PretrainedCheckpoint; Bank = $PretrainedBank },
            @{ Order = "10"; Label = "random_offset_decay"; Mode = "learned_topk_offset_decay_horizon"; Checkpoint = $RandomCheckpoint; Bank = $RandomBank }
        )
