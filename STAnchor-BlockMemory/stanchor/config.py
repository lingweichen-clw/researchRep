...
class ExperimentConfig:
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            raise ValueError(
                "candidate_key_context_mha_router requires candidate_ranking='learned_key'"
            )
        if (
            self.target.calibrator_arch == "candidate_key_context_mha_router"
        ...
        if self.target.calibrator_arch not in {"legacy", "base_as_candidate", "trajectory_conditioned_base_as_candidate", "transformer_candidate_router", "retrieval_aware_mha_router", "candidate_key_context_mha_router"}:
        ...
        if self.target.candidate_key_bottleneck_dim <= 0:
