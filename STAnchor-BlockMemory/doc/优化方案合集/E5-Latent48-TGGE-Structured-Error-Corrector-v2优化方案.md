...
本次调整只解决一个明确的容量失衡问题：当前检索侧约 39 万参数，而后置修正侧只有约 0.28 万参数。v2 将检索侧压缩到约 30 万参数，同时把修正头扩展到约 22 万参数。扩容不通过堆叠无明确用途的注意力或 MLP 完成，而是把参数分配给时间动态建模、局部图传播、历史条件非局部路由、基础风险估计和检索证据融合。
...
| 部分 | 结构 | 估算参数量 |
|---|---|---:|
| 基础风险分支 | 24 -> 256 -> 128 -> 12 | 40,844 |
| 检索证据分支 | 9 -> 128 -> 128 | 17,792 |
| 联合映射 | 256 -> 256 | 65,792 |
| 联合门控 | 256 -> 256 | 65,792 |
| horizon 输出头 | 256 -> 128 -> 1 | 33,025 |
| 9 个单特征 shape function | 1 -> 32 -> 1 | 873 |
| horizon 上限和偏置 | 每 horizon 一个标量 | 24 |
| **Structured Error Corrector** |  | **224,142** |
